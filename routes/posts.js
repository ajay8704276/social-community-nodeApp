...
exports.getPosts = function (req, res, next) {
  ...
  req.db.Post.find(
    ...
    function (err, docs) {
      ...
      body.skip = skip;
      body.posts = posts;
      req.db.Post.count({}, function (error, total) {
...
exports.updatePost = function (req, res, next) {
  ...
  if (req.body._id && req.params.id) {
    if (req.body && req.body.action === 'like') {
      ...
      req.body &&
      req.body.action !== 'comment' &&
      req.body.action !== 'watch' &&
      req.body !== 'like' &&
      req.params.id &&
      (req.body.author.id === req.session.user._id || req.session.user.admin)
    ) {
    ...
    } else if (!anyAction) next(new Error('Something went wrong.'));
  } else {
    next(new Error('No post ID.'));
  }
};
