...
let angelListClientId = null;
let angelListClientSecret = null;

if (process.env.NODE_ENV === 'production') {
  angelListClientId = process.env.ANGELLIST_CLIENT_ID;
  angelListClientSecret = process.env.ANGELLIST_CLIENT_SECRET;
} else {
  angelListClientId = process.env.ANGELLIST_CLIENT_ID_LOCAL;
  angelListClientSecret = process.env.ANGELLIST_CLIENT_SECRET_LOCAL;
}

exports.angelList = function (req, res) {
  // req.session.destroy();
  res.redirect(
    `https://angel.co/api/oauth/authorize?client_id=${angelListClientId}&scope=email&response_type=code`
  );
};
    ...
    function (httpsResponse) {
      ...
      httpsResponse.on('end', function () {
        ...
        if (data) {
          ...
          next();
        } else next(new Error('No data from AngelList'));
      });
    }
  );
  httpsRequest.end();
